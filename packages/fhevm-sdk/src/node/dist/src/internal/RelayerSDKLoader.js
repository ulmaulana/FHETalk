        ...
        return new Promise((resolve, reject) => {
            ...
            if (existingScript) {
                ...
                existingScript.addEventListener('load', () => {
                    ...
                    if (!resolved) {
                        ...
                        if (!sdk || !isFhevmRelayerSDKType(sdk, this._trace)) {
                            resolved = true;
                            cleanup();
                            reject(new Error("RelayerSDKLoader: window object does not contain a valid relayerSDK or RelayerSDK object."));
                        }
                    }
                });
                existingScript.addEventListener('error', () => {
                    if (!resolved) {
                        resolved = true;
                        cleanup();
                        reject(new Error(`RelayerSDKLoader: Failed to load SDK script from existing script tag.`));
                    }
                });
                ...
                setTimeout(() => {
                    if (!resolved) {
                        ...
                        const sdk = window.RelayerSDK || window.relayerSDK;
                        if (!sdk || !isFhevmRelayerSDKType(sdk, this._trace)) {
                            reject(new Error("RelayerSDKLoader: Timeout waiting for SDK script to load. This might be a network issue or the CDN might be blocked. Try copying WASM files locally using 'pnpm run copy-wasm'."));
                        }
                    }
                }, 5000);
                return;
            ...
            script.onload = () => {
                ...
                if (!sdk || !isFhevmRelayerSDKType(sdk, this._trace)) {
                    ...
                    reject(new Error(`RelayerSDKLoader: Relayer SDK script has been successfully loaded from ${SDK_CDN_URL}, however, the window.relayerSDK or window.RelayerSDK object is invalid.`));
                }
                // Store it in the expected location for consistency
                window.relayerSDK = sdk;
...
//# sourceMappingURL=RelayerSDKLoader.js.map