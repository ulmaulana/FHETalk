...
import { createFHEVMClientForNode, encryptValue, decryptValue, batchEncrypt, batchDecrypt, userDecryptWithSignature, publicDecrypt, shouldUseMockMode } from './utilities.js';
...
program
    ...
    try {
        ...
        const encrypted = await encryptValue(parseInt(options.value), options.publicKey, config);
...
program
    ...
    try {
        const config = loadConfig();
        const decrypted = await decryptValue(options.handle, options.contract, config, {
...
program
    ...
    try {
        const config = loadConfig();
        const values = options.values.split(',').map((v) => parseInt(v.trim()));
...
program
    ...
    try {
        const config = loadConfig();
        const handles = options.handles.split(',').map((h) => h.trim());
        const decrypted = await batchDecrypt(handles, options.contract, config, {
            ...(options.signature && { signature: options.signature }),
            usePublicDecrypt: options.public || false
        });
...
program
    ...
    try {
        const config = loadConfig();
        const decrypted = await userDecryptWithSignature(options.handle, options.contract, options.signature, config);
...
program
    ...
    try {
        const config = loadConfig();
        const decrypted = await publicDecrypt(options.handle, options.contract, config);
...
program
    ...
    try {
        ...
        try {
            ...
            if (!options.quiet)
                console.log(chalk.yellow('⚠️ FHEVM Client:'), 'Creation failed (expected in some environments)');
        }
